...
import { TeapotBufferGeometry } from "./teapot.js";
import candycane from './candycane.webp';
...
export const init = (canvas2d, canvasWebGL) => {
...
// this function is executed on each animation
    let stopped = false;
    function animate() {
        if (stopped) return;
        // controls.update();
        if (options.texturePath && !textures[options.texturePath]) {
            var texture = new THREE.TextureLoader().load(options.texturePath);
