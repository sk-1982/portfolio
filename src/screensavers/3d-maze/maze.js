...
import vertexShader from './maze.vert';
import fragmentShader from './maze.frag';
import wall from './wall.webp';
import floor from './floor.webp';
import ceiling2 from './ceiling2.webp';
import pic from './pic.webp';
import start from './start2.webp';
import fin from './fin.webp';
import open from './gl.webp';
import rat from './rat.webp';
...
export async function init(c) {
    ...
    await Promise.all([wall, floor, ceiling2, pic, start, fin, open, rat].map((url, i) => new Promise((resolve, reject) => {
        const img = new Image();
        img.src = url;
...
function mazevertices()
{
    ...
    for ( var i = 0; i < polyind.length; ++i ) {
        for ( var j = 0; j < polyind[i].length; ++j ) {
            let point = mult(scalem(.125,.125,.125),polyvert[i][polyind[i][j]]);
            pointsArray.push( point );
            let color=Math.floor(j/(3+6*(i==3)))/40;
